    ...
    for user in users:
        ...
        for student in students:
            ...
            name = (
                ...
                or " ".join(filter(None, [student.get("FirstName"), student.get("LastName")]))
                or f"Student {sid}"
            )
            child = {
                "student_id": str(sid),
...
async def _discover(hass, username: str, password: str) -> list[dict[str, str]]:
    ...
    for contact in contacts:
        ...
        children.extend(_extract_children(users, contact.school_id, contact.contact_id))
    ...
    async def async_step_user(self, user_input: dict[str, Any] | None = None):
        ...
        if user_input is not None:
            try:
                ...
                _LOGGER.exception("ARCADIA MCAS discovery failed")
    ...
    async def async_step_children(self, user_input: dict[str, Any] | None = None):
        ...
        options = [
            selector.SelectOptionDict(
                value=_child_key(child),
                label=f"{child['name']} — {child['school_name']}",
            )
    ...
    async def async_step_init(self, user_input=None):
        ...
        selected = list(
            self.config_entry.options.get(
                CONF_SELECTED_CHILDREN,
                [_child_key(child) for child in children],
            )
        )
        return self.async_show_form(
            step_id="init",
